function [ak, rk, flag] = slp_joint(x, rkm1, delta, gamma, eps_rel,...
                                Nit_max, varargin)
%
% Solves the problem
%
%   minimize_{a, r} ||Xq * a + xq - [rkm1,r] ||_2^2
%   subject to ||a||_1 <= \delta
%              ||r||_1 <= \gamma
%
% where [xq Xq] = toeplitz(x, [x(1) zeros(1, N-1)])
%
% INPUT:
%  x       : real vector (2*N x 1)
%  rkm1    : real vector (N x 1)
...
Xt = toeplitz(x, [x(1) zeros(1, N-1)]);
xq = Xt(:, 1);
Xq = Xt(:, 2:end);

% In standard form
A = [Xq, [zeros(N, N); -eye(N, N)]];
b = - xq + [rkm1; zeros(N, 1)];
